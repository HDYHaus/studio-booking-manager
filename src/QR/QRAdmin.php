...
final class QRAdmin extends AbstractAdminPage {
	...
	public function handle_download_person_qr(): void {
		...
		$filename = $this->download_filename( $person );
		nocache_headers();
	...
	private function render_person_qr(): void {
		...
		$is_print_view = ! empty( $_GET['print'] ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended -- Read-only print view flag.
		?>
		<div class="wrap sbm-admin-page <?php echo $is_print_view ? 'sbm-qr-print-view' : ''; ?>" <?php echo $is_print_view ? 'data-sbm-auto-print="1"' : ''; ?>>
			...
			<div class="sbm-card sbm-qr-output">
				<div class="sbm-qr-code" data-sbm-qr-value="<?php echo esc_attr( $url ); ?>"></div>
				<noscript><p><?php echo esc_html__( 'Enable JavaScript to render, print, or download QR codes.', 'studio-booking-manager' ); ?></p></noscript>
				...
				<p class="sbm-qr-actions">
					...
					<a class="button sbm-qr-download" href="#" data-filename="<?php echo esc_attr( $this->download_filename( $person ) ); ?>"><?php echo esc_html__( 'Download PNG', 'studio-booking-manager' ); ?></a>
					<button type="button" class="button sbm-qr-print"><?php echo esc_html__( 'Print', 'studio-booking-manager' ); ?></button>
	 ...
	 * @param int $id Person ID.
	 */
	private function print_url( int $id ): string {
	 ...
	 * @param object $person Person row.
	 */
	private function download_filename( object $person ): string {
		return sanitize_title( (string) $person->display_name ) . '-qr.png';
	}
